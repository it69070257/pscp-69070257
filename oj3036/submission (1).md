...
```text
โจทย์ให้หาว่า ถ้าเรายืนอยู่ในห้องหมายเลข N ของปราสาททรงสามเหลี่ยม เราจะต้องพังกำแพงอย่างน้อยที่สุดกี่ครั้งเพื่อเดินทางไปถึงห้องหมายเลข 1 ที่อยู่บนยอดสุด โดยมีเงื่อนไขว่าห้ามเดินทะลุออกไปนอกปราสาท

Input:
รับเลขห้องที่เราอยู่เป็นจำนวนเต็มหนึ่งตัว คือ N

Output:
พิมพ์จำนวนกำแพงที่ต้องพังน้อยที่สุดเพื่อไปให้ถึงห้อง 1 (ถ้าอยู่ที่ห้อง 1 อยู่แล้ว ให้พิมพ์ 0)

Constraints:
ค่า N มีค่าตั้งแต่ 1 ถึง 100,000 ซึ่งค่อนข้างเยอะ และโครงสร้างของห้องเป็นสามเหลี่ยมสลับทิศกัน (ชี้ขึ้นเป็นลำดับเลขคี่ ชี้ลงเป็นลำดับเลขคู่) ทำให้สเต็ปการพังกำแพงเพื่อขึ้นชั้นบนของห้องแต่ละประเภทไม่เหมือนกัน จึงต้องคำนวณหาเลขชั้นกับทิศทางของสามเหลี่ยมให้ถูกต้องก่อน
```
...
```text
Step 1: รับค่าเลขห้อง N เข้ามา
Step 2: เช็กว่าถ้า N เท่ากับ 1 ให้พิมพ์ 0 ออกมาทันทีเพราะถึงจุดหมายแล้ว
Step 3: คำนวณหาว่าห้อง N อยู่ชั้น (แถว) ไหนของปิรามิด
Step 4: เช็กว่าห้อง N เป็นสามเหลี่ยมชี้ขึ้นหรือชี้ลง โดยดูว่าลำดับในชั้นนั้นเป็นเลขคี่หรือเลขคู่
Step 5: ถ้าชี้ขึ้นพิมพ์คำตอบ 2*(r-1) แต่ถ้าชี้ลงพิมพ์ 2*r-3
```
...
```text
วิธีสุดท้ายที่ใช้จริงเหมือนกับแผนแรกเลย คือใช้สูตรคณิตศาสตร์หาเลขชั้นกับประเภทสามเหลี่ยมเอาตรงๆ โดยใช้ (N - 1) ** 0.5 หาชั้น แล้วใช้ modulo % 2 เช็กว่าสามเหลี่ยมชี้ขึ้นหรือชี้ลงเพื่อเลือกสูตรคำนวณ ที่ใช้วิธีนี้เพราะทำงานเร็วมาก ประมวลผลแค่ $O(1)$ ไม่ต้องรัน BFS วนลูปเดินทีละก้าวให้เปลืองหน่วยความจำ
```
...
```text
ทดสอบกรณีขอบเขตล่างสุด (Edge case) คืออยู่ที่ห้องหมายเลข 1 ตั้งแต่แรกอยู่แล้ว ดูว่าโปรแกรมตอบ 0 โดยไม่ต้องพังกำแพงไหม
```
...
```text
ทดสอบเคสทั่วไปที่เป็นสามเหลี่ยมทรงชี้ขึ้น (ตำแหน่งเลขคี่ในแถว) ตามตัวอย่างโจทย์ เพื่อดูว่าสูตรคำนวณกำแพงทำงานถูกต้องไหม
```
...
```text
ทดสอบเคสสามเหลี่ยมทรงชี้ลง (ตำแหน่งเลขคู่ในแถว) เพื่อดูว่าการคำนวณก้าวขึ้นชั้นบนโดยตรงถูกต้องหรือเปล่า
```
