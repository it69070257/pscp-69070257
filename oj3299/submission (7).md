...
```text
โจทย์ให้เราหาว่า หลังจากปลูกดอกไม้ไปทั้งหมด $N$ ช่องในแปลงดอกไม้ โดยเรียงไปตามแนวทแยงมุมเรื่อยๆ ดอกไม้ช่องสุดท้าย (ช่องที่ $N$) จะไปตกอยู่ใน แถบที่เท่าไหร่ ซึ่งแต่ละแถบจะมีความหนาเท่ากับ $L$ ช่อง

Input:
รับตัวเลขจำนวนเต็ม 2 ตัวในบรรทัดเดียว คั่นด้วยช่องว่าง คือ $L$ (ความหนาของแถบ) และ $N$ (จำนวนช่องดอกไม้ที่ปลูก)

Output:
พิมพ์ตัวเลขจำนวนเต็ม 1 ตัว บอกหมายเลขแถบของดอกไม้ช่องสุดท้าย
...
```text
Step 1: รับค่าอินพุตเข้ามา แล้วแยกเก็บเป็น L และ N เป็นจำนวนเต็ม
Step 2: วนลูปเพื่อไล่ปลูกทีละแนวทแยง (เริ่มจากแนวทแยงที่ 1 ซึ่งมี 1 ช่อง, แนวทแยงที่ 2 มี 2 ช่อง ไปเรื่อยๆ)
Step 3: เอาจำนวนช่องของแนวทแยงนั้นๆ ไปหักออกจาก $N$ เรื่อยๆ จนกว่า N จะหมด (<= 0) เพื่อดูว่าดอกไม้ช่องสุดท้ายตกอยู่ในแนวทแยงที่เท่าไหร่ 
Step 4: เอาลำดับแนวทแยงที่ได้มาคำนวณหาแถบ  โดยแบ่งกลุ่มตามขนาด L ด้วยสูตร แล้วพิมพ์ผลลัพธ์
```
...
```text
ทดสอบตาม Sample Testcase 1 โดยแถบกว้าง 1 (L=1) ปลูกไป 17 ช่อง (N=17) เพื่อดูว่าโปรแกรมคำนวณแนวทแยงและแถบพื้นฐานถูกต้องไหม
```
...
```text
ทดสอบตามตัวอย่างโจทย์ขวา กรณีแถบกว้างขึ้นเป็น 3 (L=3) ปลูกไป 29 ช่อง (N=29) เพื่อดูว่าการจัดกลุ่มแถบหลายๆ แนวทแยงยังคำนวณได้ถูกต้อง 
```
...
```text
ทดสอบ Edge Case กรณีปลูกแค่ช่องแรกช่องเดียว (N=1) ดูว่าโปรแกรมทำงานถูกต้องตั้งแต่แนวทแยงแรกเลยไหม
```
