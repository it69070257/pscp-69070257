...
```text
Input:
โปรแกรมจะรับข้อความเข้ามาทั้งหมด 5 บรรทัดพอดี โดยในแต่ละบรรทัดจะเป็นตัวอักษร ตัวเลข หรือช่องว่างก็ได้ ความยาวตั้งแต่ 0 ตัวอักษรขึ้นไป

Output:
บรรทัดแรกกับบรรทัดสุดท้ายจะเป็นดอกจัน (*) ปิดหัวท้ายตามความกว้างของกรอบ   อีก 5 บรรทัดตรงกลางจะเป็นข้อความที่เรารับเข้ามา โดยมีดอกจันปิดซ้าย-ขวา และเว้นวรรค (space) ระหว่างข้อความกับดอกจันฝั่งละ 1 ช่องพอดี  

Constraints:
อินพุตมา 5 บรรทัดเสมอ แต่ความยาวแต่ละบรรทัดไม่เท่ากัน เลยต้องหาความยาวของบรรทัดที่ยาวที่สุดก่อนเพื่อเอาไปคำนวณความกว้างกรอบ
```
...
```text
Step 1: รับค่าข้อความเข้ามาเก็บไว้ใน List ให้ครบ 5 บรรทัด
Step 2: หาความยาวของข้อความบรรทัดที่ยาวที่สุด เพื่อเอามาใช้เป็นเกณฑ์ตั้งความกว้างของกรอบ
Step 3: พิมพ์กรอบดอกจันแถวบนสุด โดยใช้จำนวนดอกจันเท่ากับ (ความยาวมากสุด + 4)
Step 4: วนลูปนำข้อความแต่ละบรรทัดมาพิมพ์ โดยเติม space ด้านหลังให้ยาวเท่ากับบรรทัดที่ยาวที่สุด แล้วปิดหัวท้ายด้วยดอกจันและ space
Step 5: พิมพ์กรอบดอกจันแถวล่างสุดเหมือนกับแถวบน
```
...
```text
ทดสอบกรณีทั่วไปที่ข้อความมีความยาวต่างกันปานกลาง ดูว่ากรอบขยายตามบรรทัดที่ยาวที่สุดและจัดช่องว่างได้เป๊ะไหม
```
...
```text
ทดสอบ Edge Case กรณีที่มีบรรทัดว่างเปล่า (ความยาว 0) รวมอยู่ด้วย ดูว่าโปรแกรมยังทำงานได้ปกติ ไม่พัง และจัดกรอบช่องว่างตรงบรรทัดว่างได้ถูกต้อง
```
...
```text
ทดสอบกรณีที่มีข้อความยาวๆ รวมกับตัวเลข ช่องว่าง และเครื่องหมายพิเศษ ดูว่าขนาดกรอบคำนวณถูกและไม่เบี้ยว
```
